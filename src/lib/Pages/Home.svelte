<script lang="ts">
    ...
    const updateUSPS = async () => {
        if (values.address.length > 0 || values.suiteOrApt.length > 0 || values.city.length > 0 ||values.state.length > 0 || values.zip.length > 0) {
...
<div class="section">
    ...
    <div class="text-lg md:text-2xl px-16 md:px-0 text-center">Enter an Address for your Pickup Request</div>

    <!-- Address form -->
    <div class="flex flex-col gap-3 mt-4">
        
        <input class="w-full {values.address ? 'border-color-green-primary' : 'border-gray-300'}"
            placeholder="Street"
            on:input={(e) => {
        ...
        <input class="w-full {values.suiteOrApt ? 'border-color-green-primary' : 'border-gray-300'}"
        ...
        <div class="flex flex-row gap-3">
            <input class="grow {values.city ? 'border-color-green-primary' : 'border-gray-300'}"
                placeholder="City"
                on:input={(e) => {
            ...
            <input class="w-1/5 {values.state ? 'border-color-green-primary' : 'border-gray-300'}"
                placeholder="State"
                on:input={(e) => {
            ...
            <input class="w-1/5 {values.zip ? 'border-color-green-primary' : 'border-gray-300'}"
                placeholder="Zip"
                on:input={(e) => {
